<x-app-layout>
    <div class="container">
        <a href="/report/show">
        <div class="card mt-3">
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                        <img src="https://pontas.id/wp-content/uploads/2021/04/1617228845-picsay.jpg" alt="ini foto">
                    </div>
                    <div class="col-md-8">
                        <h4>Desa Depok</h4>
                        <span class="badge text-bg-danger">Belum Selesai</span>
                        <p>Mamat Suramat . 11/06/2023 . Kota Depok</p>
                        <p>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Tempora mollitia fuga hic repellat ducimus reprehenderit nulla asperiores animi nihil et debitis, corrupti, nam voluptates autem itaque! Velit magnam omnis dolores? Lorem ipsum dolor sit amet, consectetur adipisicing elit. Recusandae, aliquid? Dignissimos amet, recusandae accusamus natus aliquid fugit sunt veniam sed voluptatibus laudantium, minima aspernatur pariatur incidunt! Magni quo nemo accusantium.</p>
                    </div>
                </div>
            </div>
        </div>
        <div class="card mt-3">
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                        <img src="https://assets.ayobandung.com/crop/0x0:0x0/750x500/webp/photo/2022/07/03/3721826091.jpg" alt="ini foto">
                    </div>
                    <div class="col-md-8">
                        <h4>Desa Sukabumi</h4>
                        <span class="badge text-bg-danger">Belum Selesai</span>
                        <p>Ricky Gerung . 11/07/2023 . Sukabumi</p>
                        <p>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Tempora mollitia fuga hic repellat ducimus reprehenderit nulla asperiores animi nihil et debitis, corrupti, nam voluptates autem itaque! Velit magnam omnis dolores? Lorem ipsum dolor sit amet, consectetur adipisicing elit. Recusandae, aliquid? Dignissimos amet, recusandae accusamus natus aliquid fugit sunt veniam sed voluptatibus laudantium, minima aspernatur pariatur incidunt! Magni quo nemo accusantium.</p>
                    </div>
                </div>
            </div>
        </div>
        <div class="card mt-3">
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                        <img src="https://bangsaonline.com/images/uploads/berita/700/86112596a2cdc3b50fb845d0a30308e5.jpg" alt="ini foto">
                    </div>
                    <div class="col-md-8">
                        <h4>Desa Cianjur</h4>
                        <span class="badge text-bg-danger">Belum Selesai</span>
                        <p>Ahmad Soleh . 11/07/2023 . Kota Cianjur</p>
                        <p>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Tempora mollitia fuga hic repellat ducimus reprehenderit nulla asperiores animi nihil et debitis, corrupti, nam voluptates autem itaque! Velit magnam omnis dolores? Lorem ipsum dolor sit amet, consectetur adipisicing elit. Recusandae, aliquid? Dignissimos amet, recusandae accusamus natus aliquid fugit sunt veniam sed voluptatibus laudantium, minima aspernatur pariatur incidunt! Magni quo nemo accusantium.</p>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </div>
</x-app-layout>